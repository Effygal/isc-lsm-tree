...
class DiskLevel {
  ...
  public: // TODO make some of these private
    ...
    // TODO: add a function to filter all the runs that might contain the user key
    // follow the if statement in the function below 
    // This function returns a vector of runs that may contain the key
    vector<DiskRun<K, V>*> filter (const K &key) {
      vector<DiskRun<K, V>*> runsMayContain;
      ...
      for (int i = maxRunToSearch; i >= 0; --i){
        if (runs[i]->maxKey == INT_MIN || key < runs[i]->minKey || key > runs[i]->maxKey || !runs[i]->bf.mayContain(&key, sizeof(K))){
          ...
          runsMayContain.push_back(runs[i]);
        }
      }

      return runsMayContain;
    }
