...
.iotta.snia.org	TRUE	/	FALSE	0	infodigest	46028388c3e0445cd1f3f87b8bc9989934e1eb95
.iotta.snia.org	TRUE	/	FALSE	0	legal	true
.iotta.snia.org	TRUE	/	FALSE	0	id	1002258
.iotta.snia.org	TRUE	/	FALSE	0	user_ip	155.33.135.41
EOF
...
downloadFile() {
  file=$1
  id=$2
  url="http://server2.iotta.snia.org/traces/key-value/$id/download?type=file&sType="
  if $useWGET; then
  ...
  if [ $error -eq 0 ]; then
    ...
    if $delete; then
      ...
      if grep "internal iotta error: ip mismatch" "$file" > /dev/null; then
        echo "This script will only run on a computer with the IP address 155.33.135.41."
